...
# - Queries that raised errors/warnings - Query, number of executions,
#    errors, warnings
# - Slow queries - Query, number of executions, execution time (total,max,avg),
#    rows sent (total, avg), scanned rows
#
# To make it work copy the config file to the configuration directory
#    and edit it with the right user and password. Then copy the .py file
#    on the collectors directory
#
...
#### Grants
grant select on performance_schema.* to graphite@localhost
...
class MysqlPSCollector(diamond.collector.Collector):
    ...
    def connect(self):
        try:
            ...
            self.log.error('MySQLPSCollector couldnt connect to database %s',
                           e)
            return False
    ...
    def fetch_number_of_reads_per_index(self):
        ...
        try:
            ...
            for row in result.fetchall():
                # Clean the digest string
                clean_digest = self.clean_string('{}_{}_{}'.
                                                 format(row['object_schema'],
                                                        row['object_name'],
                                                        row['index_name']))
                queries["number_of_reads_per_index_"+clean_digest] = \
                    row['rows_read']
    ...
    def collect(self):
        ...
        if self.is_ps_enabled():
            ...
            for key in queries:
                self.publish('mysql.performance_schema.'
                             'slow_query.'+key,
                             queries[key])
